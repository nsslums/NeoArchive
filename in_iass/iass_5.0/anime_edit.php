...
if(isset($_GET['edit'])){
    ...
    foreach($info as $cool_key=>$cool){
        //name2
        if(isset($_POST['IA_name2_'.$i])){
            $name2=htmlspecialchars($_POST['IA_name2_'.$i], ENT_QUOTES);
            $info_key[$i]=$name2;
        }else{
            error('$_POSTデータが取得できません。IA_name2'.$i);
        }
        //delete
        if(isset($_POST['IA_cool_delete_'.$i])){
            $info_delete[$i]=htmlspecialchars($_POST['IA_cool_delete_'.$i], ENT_QUOTES);
        }else{
            error('$_POSTデータが取得できません。IA_cool_delete');
        }
        //表示順
        if(isset($_POST['IA_cool_display_'.$i])){
            $info_display[$i]=htmlspecialchars($_POST['IA_cool_display_'.$i], ENT_QUOTES);
        }else{
...
foreach($info as $cool_key=>$cool){
    ...
    //group2
    $html.='<p>アニメ名2</p>
    <input type="text" name="IA_name2_'.$i.'" value="'.$cool_key.'" maxlength="64" required><br>
    <select name="IA_cool_display_'.$i.'">
        '.$cool_display.'
    </select>
    <select name="IA_cool_delete_'.$i.'">
        <option value="0">削除(未選択)</option>
        <option value="1">削除(実行)</option>
    </select><br>';
    if($cool!=null){
